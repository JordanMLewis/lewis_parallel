...
const char* Invert::getSourceName() { return "(Invert)";}
...
void Invert::Execute(){
	Logger::LogEvent("Invert: about to update input1");
	if(this->getInput() != NULL && this->getInput()->hasNotBeenUpdated()){ 
		this->getInput()->Update();
	}  
	Logger::LogEvent("Invert: done updating input1");
	...
	Logger::LogEvent("Invert: about to execute");
	...
	for(int i = 0; i < h; i++){
		for(int j = 0; j < w; j++){
			index = i*w+j;
			outputPixels[index].r = 255 - inputPixels[index].r; 
			outputPixels[index].g = 255 - inputPixels[index].g; 
			outputPixels[index].b = 255 - inputPixels[index].b; 
		}
	}	
}
...
Color::Color(int w, int h, unsigned char r, unsigned char g, unsigned char b){	
	...
	if(w > 0 && h > 0 && (255 >= r >= 0) && (255 >= g >= 0) && (255 >= b >= 0)) {
	       	this->colorWidth = w;
		this->colorHeight = h;
		this->rVal = r;
		this->gVal = g;
...
void Color::Execute(void){
	...
	if(this->colorWidth <= 0 || this->colorHeight <= 0 || ( 255 < this->rVal < 0 )|| (255 < this->gVal < 0) || (255 < this->bVal < 0)){
		char msg[1024];
